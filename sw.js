...
self.addEventListener('install',function(){
       //guarda archivos iniciarles 
        caches.open(CACHE_NAME).then(function(cache){
        cache.addAll(['/index.html','/dist/javascript/bundle.js','../public/image/banner.jpg'])
    })
})
...
function searchInCacheOrMakeRequest(request){
   ...
   return Promise.all([cachePromise,matchPromise]).then(function([cache,cacheResponse]){
      let fetchPromise= fetch(request).then(function(fetchResponse){
